...
# 국가별 최다 판매 상품 2개
SELECT Description,count(Description) CNT FROM plt.retail_on_top10 group by Description;
...
select country,sum(UnitPrice*quantity) total_s from plt.retail_on_top10 group by 1;
...
,TIMESTAMPDIFF(month,lead(invoiceDate) over(partition by customerid order by invoicedate),invoicedate) month_diff 
from plt.retail_on_top10;


 
drop table plt.m_diff;
create table plt.m_diff as
select *,date_format(invoiceDate, '%m') M,
lead(invoiceDate) over(partition by customerid order by invoicedate) next_d
,TIMESTAMPDIFF(month,invoicedate,
lead(invoiceDate) over(partition by customerid order by invoicedate)) month_diff 
from plt.retail_on_top10;
...
from plt.m_diff where country='United Kingdom' group by 1,2 ;